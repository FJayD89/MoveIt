...
def moveCheck(board, move, last_move):
    ...
    Xlow = block.x - (1 - block.vertical)
    Ylow = block.y - block.vertical
    if move[1] == 0:
        # if it isn't the inverse of what you just did
        if not last_move == [block.name, 1-move[1]]:
            # check if not touching low/high wall
            relevantSize = (1 - block.vertical) * board.width + block.vertical * board.height
            relevantHigh = block.length + (1 - block.vertical) * block.x + block.vertical * block.y
            if (1 - block.vertical) * xNew + block.vertical * yNew >= 0:
                # check if space empty
                if board.board[Xlow][Ylow] == '.':
                    return 1
    elif move[1] == 1:
        # if it isn't the inverse of what you just did
        if not last_move == [block.name, 0]:
            ...
            if not relevantSize == relevantHigh:
                # check if space empty
                if board.board[Xlow + (1 - block.vertical) * (block.length + 1)] \
                        [Ylow + block.vertical * (block.length + 1)] == '.':
                    return 1
    return 0
